...
const (
	...
	SELECT_ALL_STORE_NON_AKTIF  = "SELECT ts.store_id,ts.store_name,ts.store_address,ts.store_owner,ts.store_status,ts.store_username,ts.store_password,ts.store_images,ts.qr_path,tsc.store_category_id,tsc.store_category_name FROM tb_store ts JOIN tb_store_category tsc ON ts.store_category_id=tsc.store_category_id WHERE ts.store_status = 'NA'"
	CHANGE_ACTIVE_STORE         = "UPDATE tb_store SET store_status = 'A' WHERE store_id = ?"
	...
	SELECT_ALL_TRANSACTION_BY_USER  = "SELECT tr.transaction_id,tr.order_id,tr.user_id,tu.user_firstname,ts.store_name,tr.amount,tr.transaction_created,tr.transaction_status FROM tb_transaction tr JOIN tb_order o ON tr.order_id=o.order_id JOIN tb_user tu ON tr.user_id=tu.user_id JOIN tb_store ts ON o.store_id=ts.store_id WHERE tr.user_id = ? AND tr.transaction_status = 'Unpick' ORDER BY tr.transaction_created"
